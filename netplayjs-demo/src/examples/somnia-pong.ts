...
export class SomniaPong extends Game {
  ...
  checkGameOver(): boolean {
    const timeUp = (performance.now() - this.matchStartTime) >= MATCH_MILLIS;
    const pointsDone = (this.leftScore >= MATCH_POINTS || this.rightScore >= MATCH_POINTS);
    
    if ((timeUp || pointsDone) && !this.gameOver) {
      this.gameOver = true;
      
      // Determine winner
      let winnerAddress = '0x0000000000000000000000000000000000000000'; // default if unknown
      
      // Try to get player addresses if available
      try {
        const me = (window as any).__somnia__?.getMyAddress?.();
        if (me) {
          // Simple mapping: if you're player 0 (left), you win if left score > right score
          // This is a simplified approach - in a real implementation you'd track both players
          if (this.leftScore > this.rightScore) {
            winnerAddress = me; // Assuming player 0 is left paddle
          }
        }
      } catch (e) {
        console.log('Could not determine winner address:', e);
      }
  ...
  tick(playerInputs: Map<NetplayPlayer, DefaultInput>): void {
    ...
    let dt = SomniaPong.timestep / 1000;
    ...
    // Also check for time-based game over
    this.checkGameOver();
  }
  ...
  draw(canvas: HTMLCanvasElement) {
    ...
    // Draw time remaining
    const timeRemaining = Math.max(0, Math.floor((MATCH_MILLIS - (performance.now() - this.matchStartTime)) / 1000));
    ctx.font = "20px Arial";
    ctx.fillText(
    ...
    if (this.gameOver) {
      ...
      ctx.fillText(winnerText, PONG_WIDTH / 2, PONG_HEIGHT / 2);
...
new RollbackWrapper(SomniaPong).start();
